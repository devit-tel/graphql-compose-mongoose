...
*/
export function getAttributesFromModel(
...
export function getUniqueAttributes(mongooseModel: MongooseModel): IndexT[] {
  const indexedFields = [];
  ...
  if (Array.isArray(Object.keys(mongooseModel.schema.paths))) {
    Object.keys(mongooseModel.schema.paths).forEach(idxData => {
      const idxFields = idxData[0];
...
export type extendByReversedAttributesOpts = {
  reversedFirst?: boolean, // false by default
};

export function extendByReversedAttributes(indexes: IndexT[], opts: extendByReversedAttributesOpts = {}) {
