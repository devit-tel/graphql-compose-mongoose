...
exports.getAttributesFromModel = getAttributesFromModel;
exports.getUniqueAttributes = getUniqueAttributes;
exports.extendByReversedAttributes = extendByReversedAttributes;

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function isSpecificAttributes(idx) {
...
function getAttributesFromModel(mongooseModel) {
  ...
  if (Array.isArray(mongooseModel.schema._indexes)) {
    mongooseModel.schema._indexes.forEach(function (idxData) {
      ...
      if (!skipSpecificIndexes || !isSpecificAttributes(idxFields)) {
        if (!extractCompound) {
...
function getUniqueAttributes(mongooseModel) {
  var indexedFields = [];
  ...
  // scan compound and special indexes [MONGOOSE SCHEMA LEVEL INDEXES]
  if (Array.isArray(Object.keys(mongooseModel.schema.paths))) {
    Object.keys(mongooseModel.schema.paths).forEach(function (idxData) {
      var idxFields = idxData[0];
      var idxCfg = idxData[1];
      if (idxCfg.unique && !isSpecificAttributes(idxFields)) {
        indexedFields.push(idxFields);
      }
    });
  }
...
function extendByReversedAttributes(indexes) {
  var opts = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
